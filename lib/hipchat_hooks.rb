...
class NotificationHook < Redmine::Hook::Listener
  ...
  def controller_issues_new_after_save(context = {})
    ...
    assignee = CGI::escapeHTML(issue.assigned_to.name)
    priority = CGI::escapeHTML(issue.priority.name)
    url     = get_url(issue)
    text    = "<b>#{author}</b> reported <b><a href=\"#{url}\">#{project.name}</a></b></br>
    <b>Status:</b> #{status}</br><b>Priority: </b>#{priority}</br> <b>Tracker:</b> #{tracker} <a href=\"#{url}\">##{issue.id}</a></br>
    <b>Asignee: </b>#{assignee}</b></br> <b>Subject: </b>#{subject}</br>"
  ...
  def controller_issues_edit_after_save(context = {})
    ...
    url     = get_url(issue)
    text    = "<b>#{author}</b> updated <b><a href=\"#{url}\">#{project.name}</a></b></br>
    <b>Status:</b> #{status}</br><b>Priority: </b>#{priority}</br> <b>Tracker:</b> #{tracker} <a href=\"#{url}\">##{issue.id}</a></br>
    <b>Asignee: </b>#{assignee}</b></br> <b>Subject: </b>#{subject}</br>"
    text   += "<b>Notes:</b> <i>#{truncate(comment)}</i>" unless comment.blank?
